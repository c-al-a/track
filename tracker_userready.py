...
FILE_PATH = r"\\ntphfs\cpid$\Project Management\Ops Tracker\Resource_Tracker_Link_Path\Resource Tracker_New.xlsm"
# FILE_PATH = "Resource Tracker_New (4).xlsm"
...
# =========================
def build_employee_details_table(employee_list, resources_df):
    ...
    details_df = resources_df[cols_needed].copy()

    details_df = details_df[
        details_df["NAME"].isin(employee_list)
    ]

    details_df = details_df.sort_values("NAME")
...
# =========================
if mode == "Employee":
    name_input = st.text_input("Name Search (timeline)")
    ...
    if data:
        ...
        # --- STEP 1: Add search box for final table ---
        st.subheader("Resources Details")
        search_text = st.text_input("Search Details")
        
        # --- STEP 2: Build the employee details table ---
        employee_details_df = resources[resources["NAME"].isin(matched)][
            ["EMPLOYEE CODE", "NAME", "DESIGNATION", "BUSINESS LINE", "TEAM CLASSIFICATION", "CORE TEAM/CLIENT", "CORE PROJECT MANAGER", "STATUS", "REMARKS"]
        ]

        # Filter rows based on search
        if search_text:
            employee_details_df = employee_details_df[
                employee_details_df.apply(
                    lambda row: row.astype(str).str.contains(search_text, case=False).any(),
                    axis=1
                )
            ]
        
        # --- STEP 3: Display the filtered table ---
        st.dataframe(employee_details_df, width = "content", hide_index=False)
...
# =========================
    ...
    if team_input:
        ...
        if not matched:
            ...
            # =========================
            st.subheader("Resources in Team")

            employee_details_df = build_employee_details_table(
                matched,
                resources
            )
